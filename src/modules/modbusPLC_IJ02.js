...
client1.on('connect', async function() {
    console.log("PLC scaraIJ02 connected");
    ...
    while (_PLC.statusModbusPLC) {
        client1.readHoldingRegisters(0, 13).then(function(resp) {
            ...
            _ICI02.lotCounter.input = data[0] + data[1]*65536;
            _ICI02.lotCounter.output = data[2] + data[3]*65536;
            _ICI02.shiftCounter.input = data[4] + data[5]*65536;
            _ICI02.shiftCounter.output = data[6] + data[7]*65536;
            _ICI02.lotCounter.rejected = data[8];    
            _ICI02.shiftCounter.rejected = data[9];
            _ICI02.lotCounter._Jinguan02.rejected = data[10];
            _ICI02.shiftCounter._Jinguan02.rejected = data[11];
            _ICI02.cardsContainer = data[12];

            // console.log('ICI : ', _ICI02);

            //console.log(resp);            
        }, function(err) {
            console.error;
            console.log('PLC IJ02 | Error Reading Holding Registers: 0 -> 12');
            emitter.emit('errorLog', {
                info: 'Error | PLC IJ02 | Reading Holding Register 0 : 12 |',
                code: err
            });
        });

        client1.writeSingleRegister(13, _scaraIJ02.setPointCards).then(function(resp){
            // console.log('Writting Register: 21 -> Value: ', _scaraIJ02.setPointCards);
            //console.log(resp);
        }, function(err) {
            console.error;
            console.log('PLC IJ02 | Error Writing Single Register: 13');
            emitter.emit('errorLog', {
                info: 'Error | PLC IJ02 | Writing Single Register 13 |',
                code: err
            });
        });
        ...
        client1.readCoils(0,2).then(function(resp) {
            ...
            //console.log(resp);
            let data = resp.coils;
            _ICI02.status.relaySafety = data[0];
            _ICI02.status.machine = data[1];
        }, function(err) {
            console.error;
            console.log('PLC IJ02 | Error Reading Coils: 0 -> 1');
            emitter.emit('errorLog', {
                info: 'Error | PLC IJ02 | Reading Coils 0 : 1 |',
                code: err
            });
        });

        client1.writeMultipleCoils(2,[_scaraIJ02.openGripper,_resetLotCardsCounters,_resetShiftCardsCounters]).then(function(resp) {
            // console.log('Writing Bool: 2 -> 4');
            //console.log(resp);
        }, function(err) {
            console.error;
            console.log('PLC IJ02 | Error Writing Coils: 2 -> 4');
            emitter.emit('errorLog', {
                info: 'Error | PLC IJ02 | Writing Coils 2 : 4 |',
                code: err
            });
        });
        ...
        client1.readCoils(99,116).then(function(resp) {
            ...
            console.error;
            console.log('PLC IJ02 | Error Reading Coils: 99 -> 115');
            emitter.emit('errorLog', {
                info: 'Error | PLC IJ02 | Reading Coils 99 : 115 |',
                code: err
            });
        });
        ...
        client1.readCoils(119,133).then(function(resp) {
            ...
            console.error;
            console.log('PLC IJ02 | Error Reading Coils: 119 -> 132');
            emitter.emit('errorLog', {
                info: 'Error | PLC IJ02 | Reading Coils 119 : 132 |',
                code: err
            });
        });

        if (counterSeconds == 600) {
            _ICI02.status.machine ? _ICI02.timeWorking.machineOn++ : _ICI02.timeWorking.machineOff++
            counterSeconds = 0;
        }

        machinesData().upDateMachine(_ICI02);
        await delay(100);   
        counterSeconds ++;
    }
});  
...
client1.on('error', (err) => {
    console.log('Error on IJ02 | Error : ', err);
    _PLC.statusModbusPLC = false;
    client1.reconnect();
    emitter.emit('errorLog', {
        info: 'Error | PLC IJ02 | Connection Failed |',
        code: err
    });
});
...
module.exports = function (task, value) {
    if (task == 'setPointCards'){
        _scaraIJ02.setPointCards = parseInt(value);
    }
    if (task == 'openGripper'){
        _scaraIJ02.openGripper = value;
    }  
    ...
    if (task == 'resetShiftCardsCounters'){
        ...
        _ICI02.timeWorking.machineOn = 0;
        _ICI02.timeWorking.machineOff = 0;
    } else if (task == 'timeWorking') {
        _ICI02.timeWorking.machineOn = value[0];
        _ICI02.timeWorking.machineOff = value[1];
    }
    return _PLC;
}
