...
client1.on('connect', async function() {
    console.log("PLC scaraJM01 connected");
    ...
    while (_PLC.statusModbusPLC) {
        client1.readHoldingRegisters(0, 19).then(function(resp) {
            ...
            _Jinguan01.lotCounter.input = data[0] + data[1]*65536;
            _Jinguan01.lotCounter.output = data[2] + data[3]*65536;
            _MPR01.lotCounter.input = data[4] + data[5]*65536;
            _MPR01.lotCounter.output = data[6] + data[7]*65536;
            _Jinguan01.shiftCounter.input = data[8] + data[9]*65536;
            _Jinguan01.shiftCounter.output = data[10] + data[11]*65536;
            _MPR01.shiftCounter.input = data[12] + data[13]*65536;
            _MPR01.shiftCounter.output = data[14] + data[15]*65536;
            _MPR01.lotCounter.rejected = data[16];    
            _MPR01.shiftCounter.rejected = data[17];
            _Jinguan01.cardsContainer = data[18];

            // console.log('Jinguan : ', _Jinguan01);

            //console.log(resp);            
        }, function(err) {
            console.error;
            console.log('PLC JM01 | Error Reading Holding Registers: 0 -> 18');
            emitter.emit('errorLog', {
                info: 'Error | PLC JM01 | Reading Holding Register 0 : 18 |',
                code: err
            });
        });

        client1.writeSingleRegister(19, _scaraJM01.setPointCards).then(function(resp){
            // console.log('Writting Register: 21 -> Value: ', _scaraJM01.setPointCards);
            //console.log(resp);
        }, function(err) {
            console.error;
            console.log('PLC JM01 | Error Writing Single Register: 19');
            emitter.emit('errorLog', {
                info: 'Error | PLC JM01 | Writing Single Register 19 |',
                code: err
            });
        });
        ...
        client1.readCoils(0,3).then(function(resp) {
            ...
            //console.log(resp);
            let data = resp.coils;
            _Jinguan01.status.relaySafety = data[0];
            _Jinguan01.status.machine = data[1];
            _MPR01.status.machine = data[2]
        }, function(err) {
            console.error;
            console.log('PLC JM01 | Error Reading Coils: 0 -> 2');
            emitter.emit('errorLog', {
                info: 'Error | PLC JM01 | Reading Coils 0 : 2 |',
                code: err
            });
        });

        client1.writeMultipleCoils(3,[_scaraJM01.openGripper,_resetLotCardsCounters,_resetShiftCardsCounters]).then(function(resp) {
            // console.log('Writing Bool: 2 -> 4');
            //console.log(resp);
        }, function(err) {
            console.error;
            console.log('PLC JM01 | Error Writing Coils: 3 -> 5');
            emitter.emit('errorLog', {
                info: 'Error | PLC JM01 | Writing Coils 3 : 5 |',
                code: err
            });
        });
        ...
        client1.readCoils(99,119).then(function(resp) {
            ...
            console.error;
            console.log('PLC JM01 | Error Reading Coils: 99 -> 118');
            emitter.emit('errorLog', {
                info: 'Error | PLC JM01 | Reading Coils 99 : 118 |',
                code: err
            });
        });
        ...
        client1.readCoils(119,133).then(function(resp) {
            ...
            console.error;
            console.log('PLC JM01 | Error Reading Coils: 119 -> 132');
            emitter.emit('errorLog', {
                info: 'Error | PLC JM01 | Reading Coils 119 : 132 |',
                code: err
            });
        });
        
        if (counterSeconds == 600) {
            _Jinguan01.status.machine ? _Jinguan01.timeWorking.machineOn++ : _Jinguan01.timeWorking.machineOff++
            _MPR01.status.machine ? _MPR01.timeWorking.machineOn++ : _MPR01.timeWorking.machineOff++
            counterSeconds = 0;
        }

        await machinesData().upDateMachine(_Jinguan01);
        await machinesData().upDateMachine(_MPR01);
        await delay(100);   
        counterSeconds ++;
...
client1.on('error', (err) => {
    ...
    client1.reconnect();
    emitter.emit('errorLog', {
        info: 'Error | PLC JM01 | Connection Failed |',
        code: err
    });
});
...
module.exports = function (task, value) {
    if (task == 'setPointCards'){
        _scaraJM01.setPointCards = parseInt(value);
    }
    if (task == 'openGripper'){
        _scaraJM01.openGripper = value;
    }  
    ...
    if (task == 'resetShiftCardsCounters'){
        ...
        _Jinguan01.timeWorking.machineOn = 0;
        _Jinguan01.timeWorking.machineOff = 0;
        _MPR01.timeWorking.machineOn = 0;
        _MPR01.timeWorking.machineOff = 0;
    } else if (task == 'timeWorkingJinguan') {
        _Jinguan01.timeWorking.machineOn = value[0];
        _Jinguan01.timeWorking.machineOff = value[1];
    } else if (task == 'timeWorkingMPR') {
        _MPR01.timeWorking.machineOn = value[0];
        _MPR01.timeWorking.machineOff = value[1];
    }
    return _PLC;
}
